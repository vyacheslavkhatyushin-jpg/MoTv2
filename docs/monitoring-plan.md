...
3. ✅ **Страница `/<project>/monitoring`** — отдельная страница (не вкладка в редакторе): тот же 3D-движок и снимок проекта, но урезанный интерфейс — только зелёное/красное неоновое свечение на модели, список активных аварий и история, без редактирования. Живые обновления по WebSocket-каналу `/api/monitor/ws`. В обычном редакторе — ссылка «📡 Мониторинг», на странице мониторинга — ссылка «✏️ Редактор» обратно.
